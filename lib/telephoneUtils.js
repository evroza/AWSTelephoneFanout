...
const updateStatTok = (token, status = null) => {
	...
	let params = {}; // Holds the telephone update details sent to dynamoDB
	
	//first decode Token to verify it is valid:
	jwt.verify(token, process.env.JWT_SECRET, function(err, decoded) {
		// If invalid token --> decoded undefined, also have 'err'
		console.log(decoded.telephone.TelephoneSerial);
		if(decoded.telephone.TelephoneSerial){
			let telephoneSerial = decoded.telephone.TelephoneSerial;
				...
				if( result === true){
					...
					if( typeof status === 'boolean'){
						...
						params = {
							...
							UpdateExpression: "set JWTToken = :t, Active=:a, LastLogin=:ll",
						...
						if(status !== null) {
							...
							params = {
								...
								UpdateExpression: "set JWTToken = :t, LastLogin=:ll",
								ExpressionAttributeValues: {
									":t": token,
									":ll": new Date().getTime()
